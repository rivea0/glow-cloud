...
export default function CurrentWeather({ weatherData, city, countryName }: { 
  ...
  const weatherDescription = `${weatherCodes[weatherData.weathercode].description[0].toUpperCase()}${weatherCodes[weatherData.weathercode].description.slice(1)}`
  return (
    weatherData &&
    <section className={styles.current}>
      <div className={stylesCurrent.currentWeatherBox}>
        ...
        priority={true}
      />
      <h1 className={stylesCurrent.temperature}>{Math.round(weatherData.temperature)}
        <span className={stylesCurrent.degree}>°C</span>
      </h1>
      <p className={stylesCurrent.date}>{dateStr}</p>
      <p className={stylesCurrent.descriptionText}>
